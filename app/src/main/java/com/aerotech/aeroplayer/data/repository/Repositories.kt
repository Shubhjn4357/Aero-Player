package com.aerotech.aeroplayer.data.repository
...
import com.aerotech.aeroplayer.data.database.*
...
import com.aerotech.aeroplayer.domain.repository.IHistoryRepository
import com.aerotech.aeroplayer.domain.repository.IMediaRepository
import com.aerotech.aeroplayer.domain.repository.IPreferencesRepository
...
class MediaRepository(private val mediaDao: MediaDao) : IMediaRepository {
    ...
    override suspend fun scanMedia(context: Context): Unit = withContext(Dispatchers.IO) {
                    ...
                    if (isVideo || isAudio) {
                        ...
                        mediaList.add(
                            MediaEntity(
                                ...
                                mimeType = com.aerotech.aeroplayer.util.ContentResolverUtils.inferMimeType(fileUri, filePath, context),
                                genre = if (isVideo) "Video" else "Audio"
                            )
                        )
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                com.aerotech.aeroplayer.ui.screens.ThumbnailManager.preloadThumbnails(context, verifiedList)
            } catch (e: Exception) {}
        }
    }
}
