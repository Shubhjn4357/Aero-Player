...
import com.example.ui.viewmodel.updateDefaultPlayerEngine

/**
 * BroadcastReceiver enabling CLI / ADB control of the player:
 * Usage via ADB:
 * adb shell am broadcast -a com.example.player.VLC_CLI --es action "play"
 * adb shell am broadcast -a com.example.player.VLC_CLI --es action "pause"
 * adb shell am broadcast -a com.example.player.VLC_CLI --es action "toggle"
 * adb shell am broadcast -a com.example.player.VLC_CLI --es action "stop"
 * adb shell am broadcast -a com.example.player.VLC_CLI --es action "seek" --el position_ms 30000
 * adb shell am broadcast -a com.example.player.VLC_CLI --es action "seek_by" --el offset_ms 10000
 * adb shell am broadcast -a com.example.player.VLC_CLI --es action "engine" --es engine "VLC"
 * adb shell am broadcast -a com.example.player.VLC_CLI --es action "sub_toggle"
 * adb shell am broadcast -a com.example.player.VLC_CLI --es action "sub_delay" --el offset_ms 500
 * adb shell am broadcast -a com.example.player.VLC_CLI --es action "sub_encoding" --es encoding "UTF-8"
 * adb shell am broadcast -a com.example.player.VLC_CLI --es action "load" --es uri "/sdcard/Movies/sample.mkv"
 */
class VlcCliReceiver : BroadcastReceiver() {
    companion object {
        const val ACTION_VLC_CLI = "com.example.player.VLC_CLI"
        const val ACTION_MEDIA_CLI = "com.example.player.MEDIA_CLI"
